...
Пример:x  # Возвращает значение x
...
Пример:f x y  # Применяет f к x и y
...
Пример:let x = 5 in x + 1  # Возвращает 6
...
Пример:if x == 0 then 1 else 2



3.3.7 Сопоставление с образцом

Синтаксис:match <выражение> with
| <образец> -> <выражение>
| <образец> -> <выражение>
...
...
Пример:[1 2 3]  # Возвращает VList [VInt 1; VInt 2; VInt 2]
...
Пример:!"ls -la"  # Возвращает вывод команды ls -la
...
Пример:boundary (simplex [0 1 2])  # Возвращает Complex: [[0;1]; [0;2]; [1;2]]
...
Синтаксис:module <идентификатор>
  <объявление>*
end
...
obj
Unit
Выводит значение без новой строки


println
obj
Unit
Выводит значение с новой строкой
...
VComplex
VInt
Размерность первого симплекса (длина - 1)


faces
VComplex
VComplex
Все подсимплексы (удаление одной вершины)
...
Добавьте тесты в run_tests.fsx, используя фреймворки F# (например, Expecto).
Запустите скрипт:dotnet fsi run_tests.fsx
